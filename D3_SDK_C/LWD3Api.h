...
/// @brief 查找所有可使用的设备。从3.2.1版本开始，不再建议使用该函数，请改用“LWGetDeviceInfoList”函数。
...
/// @brief 注册网络异常检测的全局回调函数。当出现SDK与设备的网络连接异常时，该回调函数会被立即调用。
/// @note 如果设备已用LWRegisterNetworkMonitoringCallback1函数注册了回调函数，则该全局回调函数将对该设备失去作用。
...
/// @note 优先级大于全局回调函数（即用LWRegisterNetworkMonitoringCallback函数注册的全局回调函数）。
/// @param[in] handle 设备描述符。
/// @param[in] pCallback 回调函数。当指针为空时即：nullptr，则将移除已注册的回调函数。
/// @param[in] pUserData 用户数据。
/// @return 返回码。
...
/// @brief 注册探测新数据可用的全局回调函数。每当有新数据可用时，该回调函数会被立即调用。
/// @note 如果设备已用LWRegisterFrameReadyCallback1函数注册了回调函数，则该全局回调函数将对该设备失去作用。
/// @param[in] pCallback 回调函数。当指针为空时即：nullptr，则将移除已注册的回调函数。
/// @param[in] pUserData 用户数据。
/// @return 返回码。
...
/// @note 优先级大于全局回调函数（即用LWRegisterFrameReadyCallback函数注册的全局回调函数）。
/// @param[in] handle 设备描述符。
/// @param[in] pCallback 回调函数。当指针为空时即：nullptr，则将移除已注册的回调函数。
/// @param[in] pUserData 用户数据。
/// @return 返回码。
